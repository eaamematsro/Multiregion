...
class MLP(nn.Module):
    def __init__(self, layer_sizes: dict = None, non_linearity: nn.functional = None, input_size: int = 150,
        ...
        for key, default_value in default_cfg.items():
            ...
            if not ['target_cfg', 'feedback', 'trigger'] in layer_names:
                warnings.warn('layer dict was incorrectly formatted returning to default config.')
                layer_sizes = {
                    'target_cfg': [50, 25, 12],
                    'feedback': [10, 5],
                    'trigger': [1]
...
class SequentialReachingNetwork(nn.Module):
    def __init__(self, duration: int = 500, n_clusters: int = 3, n_reaches: int = 3, use_cnn: bool = False,
                 ...
                 wd: float = 1e-3, n_hidden: int = 500, **kwargs):
        super().__init__()
        ...
        if use_cnn:
            ...
            self.model_inputs = MLP(layer_sizes=layer_size, input_size=(n_reaches * 2), output_size=n_hidden)
